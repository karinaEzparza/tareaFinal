...
function queryFormSuccess(tx, results) {
	...
	$.registro = results.rows.item(0);
		...
		$("#text_nombre").val($.registro.nombre);
		$("#text_domicilio").val($.registro.domicilio);
		$("#text_telefono").val($.registro.telefono);
		$("#text_mail").val($.registro.email);
		$("#text_nota").val($.registro.nota);
...
function initForm(){
	...
	$("#cat_amigo").trigger("click").trigger("click")
}
...
function queryDBUpdateForm(tx){
	...
	tx.executeSql('UPDATE servicios SET nombre="'+$("#text_nombre").val()+'", domicilio="'+$("#text_domicilio").val()+'",telefono="'+$("#text_telefono").val()+'",email="'+$("#text_mail").val()+'",nota="'+$("#text_nota").val()+'",categoria="'+cat+'",foto = "'+$.imageURL+'" WHERE id='+$.id);
}
function updateFormSuccess(tx) {
	...
	selector.find("a:first").find("span").html($("#text_nombre").val() + " " + $("#text_domicilio").val());
...
function queryDBInsertForm(tx){
	...
	tx.executeSql("INSERT INTO servicios (nombre,domicilio,telefono,categoria,foto,nota,email) VALUES ('"+$("#text_nombre").val()+"','"+$("#text_domicilio").val()+"','"+$("#text_telefono").val()+"','"+cat+"','"+$.imageURL+"','"+$("#text_nota").val()+"','"+$("#text_mail").val()+"')", [], newFormSuccess, errorDB);
}
...
function borrar(){
	if(db != null){
		db.transaction(queryborrar, errorDB, borrarSuccess);
	}
}

function queryborrar(tx){
	var cat = $("#cajaCategorias").find("input:checked").val();
	tx.executeSql('delete from servicios WHERE id='+$.id);
}
function borrarSuccess(tx) {
